...
# -----------------------------
# Used only by compute_metrics() to number per-eval outputs during training (val, per epoch).
EVAL_CALL = 0
...
class EmotionDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        row = self.df.loc[idx]
        # Aspect conditioning is done by injecting the aspect into the input text.
...
# ===============================
# 8) METRICS (VAL DURING TRAINING)
# ===============================
def compute_metrics(eval_pred):
    """
    Trainer metrics (validation only, during training):
    - Confusion matrix (counts) saved as PNG (per epoch)
    - Classification report saved as CSV (per epoch)
    - macro_f1 returned for Trainer progress
    """
...
# ===============================
# 8.5) UNSEEN (TEST) EVAL HELPERS
# ===============================
def evaluate_and_save_unseen(trainer, dataset, split_name: str, label_names: list, results_dir: Path):
    """
    Unseen evaluation (e.g., TEST) AFTER training:
    - Uses the already-trained model weights (NO retraining).
    - Saves:
        * <split>_confusion_matrix_emotion.png
        * <split>_classification_report.csv
        * <split>_metrics.txt  (macro/weighted/micro F1)
    """
    # Be explicit: evaluation mode + no gradients = inference only
    trainer.model.eval()
    with torch.no_grad():
    ...
    # Robust F1 variants (useful to report in papers)
    ...
    csv_path = results_dir / f"{split_name.lower()}_classification_report.csv"
    report_df.to_csv(csv_path)
    ...
    plt.title(f"{split_name.upper()} Emotion Confusion Matrix (Counts)", pad=30)
    plt.xlabel("Predicted Emotion")
    plt.ylabel("Gold Emotion")
    plt.tight_layout()

    png_path = results_dir / f"{split_name.lower()}_confusion_matrix_emotion.png"
    plt.savefig(png_path, dpi=300)
    plt.close()

    # Save a small text summary for quick copy/paste
    metrics_path = results_dir / f"{split_name.lower()}_metrics.txt"
...
args = TrainingArguments(
    ...
    eval_strategy="epoch",   # keep as-is (do not change working behavior)
    save_strategy="epoch",
    learning_rate=2e-5,
...
# ===============================
# 11) UNSEEN TEST EVAL (AFTER TRAINING, NO RETRAINING)
# ===============================
# This produces robust F1 + clean, clearly named files for the unseen split.
evaluate_and_save_unseen(
    trainer=trainer,
    dataset=test_ds,
    split_name="TEST",
...
# Keep your original final test evaluation call too (does not retrain).
# It may also trigger compute_metrics outputs (depending on Trainer internals),
# but we keep it unchanged to avoid breaking anything you currently rely on.
