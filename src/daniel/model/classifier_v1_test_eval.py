...
# coding: utf-8
"""
classifier_v1_test_eval.py

Identical to classifier_v1.py with one addition:
after training completes, load test.jsonl and run a full test-set evaluation,
saving confusion matrix PNG + classification report CSV + test_metrics.txt
into the run directory alongside the existing per-epoch artifacts.

Label encoding is fit on the combined train+val data (same as original V1),
then transform() is applied to the test split.
"""
...
# ─── Run directory ───────────────────────────────────────────────────────────
EXPERIMENT_NAME = Path(__file__).stem  # classifier_v1_test_eval
...
# ─── 1) Load train.jsonl (same as V1 original) ───────────────────────────────
data_file = Path(data_root) / "train.jsonl"
rows = []
with open(data_file, "r", encoding="utf-8") as f:
    for line in f:
        rows.append(json.loads(line))

print(f"Loaded {len(rows)} rows from train.jsonl")

def light_check(rows, name):
    for i, r in enumerate(rows):
        assert isinstance(r, dict)
        assert "input" in r and isinstance(r["input"], str) and r["input"].strip()
        assert "output" in r and isinstance(r["output"], list) and len(r["output"]) > 0
        for j, o in enumerate(r["output"]):
            for k in ("aspect", "polarity", "emotion"):
                assert k in o and isinstance(o[k], str) and o[k].strip()
    print(f"{name}: {len(rows)} rows passed")

light_check(rows=rows, name="train.jsonl")
...
# ─── 2) Label encoding (fit on all train.jsonl data, same as V1) ─────────────
...
print("\nEmotion labels:", EMOTION_NAMES)
print("Polarity labels:", POLARITY_NAMES)

# ─── 3) Train / val split (same 80/20 by unique text, same seed as V1) ───────
...
# ─── 4) Load test.jsonl and apply encoders ───────────────────────────────────
test_file = Path(data_root) / "test.jsonl"
test_rows = []
with open(test_file, "r", encoding="utf-8") as f:
    for line in f:
        test_rows.append(json.loads(line))

print(f"\nLoaded {len(test_rows)} rows from test.jsonl")
...
# V1 note: test.jsonl may contain labels not seen in train; raise clearly if so
...
# ─── 7) Metrics (per-epoch, same as V1) ──────────────────────────────────────
...
print("\n" + "=" * 50)
print("TEST SET EVALUATION (test.jsonl)")
print("=" * 50)
...
for i in range(len(EMOTION_NAMES)):
    ax.add_patch(plt.Rectangle((i, i), 1, 1, fill=False, edgecolor="black", lw=2))
plt.title("Emotion Confusion Matrix — TEST SET (V1)", pad=30)
plt.xlabel("Predicted Emotion"); plt.ylabel("Gold Emotion")
plt.tight_layout()
